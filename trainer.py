...
parser.add_argument('--model', default="resnet18", type=str, help='resnet18 | resnet34 | resnet50')
...
class ClassifierTrainer(object):
    def __init__(self):
        ...
        self.true_test_dataloader = None
        self.checkpoints_dir = None
        self.scheduler = None
        self.bs = opt.bs
        self.epoch = 10
        self.lr = opt.lr
        self.train_dataloader = dataloader(batch_size=self.bs, mode="train")
        self.test_dataloader = dataloader(batch_size=self.bs, mode="test")
        ...
        self.optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.lr)  # 优化器负责更新参数
    ...
    def set_scheduler(self):
        if opt.scheduler == 'None':
            ...
            self.scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=self.optimizer, T_mult=2, T_0=5)
        elif opt.scheduler == 'linear':
            self.scheduler = optim.lr_scheduler.StepLR(optimizer=self.optimizer, step_size=1, gamma=0.9)
    ...
    def test(self, model_path=None):
        if model_path is not None:
            self.model.load_state_dict(torch.load(model_path))
            print("加载参数文件: {}".format(model_path))
        self.true_test_dataloader = dataloader(batch_size=self.bs, mode="test", true_test=True)
        self.model.eval()
        image_id_list = []
        is_male_list = []
        with torch.no_grad():
            for batch_idx, datas in tqdm.tqdm(enumerate(self.true_test_dataloader)):
        ...
        submit_data = pd.DataFrame(out_dict)
...
# path = 'checkpoints/resnet34/1.0000.pth'
# path = 'checkpoints/resnet50/1.0000.pth'
